...
class LoginGoldSpider(scrapy.Spider):
    name = 'login_Gold'
    allowed_domains = ['rztrading.com']
    start_urls = ["http://rztrading.com/login.php"]

    # Authentication to login to the website - crendentials are removed for security 
    def parse(self, response):     
        yield FormRequest('http://rztrading.com/login.php',
                            formdata ={ 'email':'xxxxxxxxxxxxxxxxxxxxxx',
                                        'password':'xxxxxx',
                                        'Login':'Login',
                                        'process':'1'},
    ...
    def parse_after_login(self,response):
        ...
        url = 'http://rztrading.com/inside/ordersDetails.php?storeOrderID=' + g
